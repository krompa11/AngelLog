(()=>{
  ...
  const status=()=>{let el=document.getElementById('aaWaterLoadStatus');if(el)return el;const map=document.getElementById('aaMap');if(!map)return null;el=document.createElement('div');el.id='aaWaterLoadStatus';el.style.cssText='position:absolute;left:14px;bottom:86px;z-index:700;background:rgba(35,35,35,.88);color:#ddd;padding:7px 10px;border-radius:6px;font-size:12px;pointer-events:none;max-width:270px';map.parentElement.appendChild(el);return el};
  ...
  async function fetchTile(t,token){
    ...
    const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),7500);
  ...
  function addWaters(waters){
    for(const w of waters){if(!w?.source_key||markerByKey.has(w.source_key)||osmMarkers.length>=190)continue;
      const m=L.marker([w.latitude,w.longitude],{icon:waterIcon(w.water_environment),keyboard:false}).addTo(aaMap);
      m.bindPopup(`<div style="min-width:190px"><b style="font-size:16px">${esc(w.name)}</b><div style="margin-top:4px;color:#777">${esc(w.water_type||'Gewässer')} · ${envLabel(w.water_environment)}</div><button style="margin-top:9px;width:100%;border:0;border-radius:5px;padding:8px;background:#58c900;color:white" onclick='aaOpenOsmWater(${JSON.stringify(JSON.stringify(w))})'>Gewässer öffnen</button></div>`);
  ...
  async function loadVisible(){
    ...
    if(z<9){removeAll();if(el)el.textContent='Zoome näher heran, um Gewässer weltweit anzuzeigen.';return}
    ...
    let finished=0;
    const jobs=tilesForBounds(b).map(async t=>{const waters=await fetchTile(t,token);if(token!==loadToken)return;addWaters(waters);finished++;if(el){if(osmMarkers.length)el.textContent=`${osmMarkers.length} Gewässer geladen${finished<4?' · weitere folgen …':''}`;else if(finished<4)el.textContent='Gewässer weltweit werden geladen …'}});
    await Promise.allSettled(jobs);if(token!==loadToken)return;
    if(el)el.textContent=osmMarkers.length?`${osmMarkers.length} Gewässer im Kartenausschnitt · weltweit`:'Keine Gewässerdaten verfügbar – zoome etwas näher heran oder verschiebe die Karte.';
  }
  window.aaOpenOsmWater=async raw=>{
    ...
    try{
      ...
      const row={
        ...
        water_environment:w.water_environment||null,
        latitude:w.latitude,
        longitude:w.longitude,
        official_url:w.website||null,
        association:w.operator||null,
        source_name:w.source_name||(w.official?'Landesamt für Umwelt Brandenburg':'OpenStreetMap'),
