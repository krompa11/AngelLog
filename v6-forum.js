(()=>{
  ...
  async function openThread(id){
    ...
    view.innerHTML=`<header class="aa-topbar"><button id="forumBack" class="forum-back">‹</button><h1>Forum</h1><div></div></header><div class="forum-thread-head"><span class="forum-chip">${safe(thread.category)}</span><h2>${safe(thread.title)}</h2><div class="forum-thread-meta"><span>${safe(authorName)}</span><span>${fmt(thread.created_at)}</span></div>${thread.water_name?`<div class="forum-water-link">≋ ${safe(thread.water_name)}</div>`:''}<p class="forum-thread-body">${safe(thread.body)}</p></div><div id="forumPosts">${(posts||[]).map(p=>postHtml(p,pm.get(p.user_id))).join('')||'<div class="forum-empty">Noch keine Antworten. Schreib die erste.</div>'}</div><div class="forum-reply"><textarea id="forumReplyText" maxlength="10000" placeholder="Deine Antwort …"></textarea><button id="forumReplyBtn" class="forum-primary">Antwort veröffentlichen</button></div>`;
    q('#forumBack').onclick=closeThread;q('#forumReplyBtn').onclick=sendReply
  }
  function postHtml(p,profile){const name=nameOf(profile);return `<article class="forum-post"><div class="forum-avatar">${avatarHtml(profile,name)}</div><div class="forum-post-copy"><b>${safe(name)}</b><small>${fmt(p.created_at)}</small><p>${safe(p.body)}</p></div></article>`}
