(()=>{
  ...
  let markers=[],rows=[],panel=null,button=null,select=null,countEl=null,statusEl=null,enabled=false,token=0,timer=null,boundMap=null,pro=false;
  ...
  function addStyles(){
    ...
    const s=document.createElement('style');s.id='aaCatchMapStyles';s.textContent=`
      ...
      .aa-catch-map-btn{border:1px solid #505253;background:rgba(31,32,33,.95);color:#fff;border-radius:999px;min-height:40px;padding:0 13px;font-weight:800;font-size:13px;box-shadow:0 8px 22px rgba(0,0,0,.28);cursor:pointer}.aa-catch-map-btn.active{border-color:#61d000;box-shadow:0 0 0 2px rgba(97,208,0,.18),0 8px 22px rgba(0,0,0,.28)}.aa-catch-map-btn.locked{border-color:#a88a24;color:#f1d16c}
  ...
  function syncAccessUi(){
    if(!button)return;button.classList.toggle('locked',!pro);button.classList.toggle('active',pro&&enabled);
    button.innerHTML=pro?`🐟 Fangorte <span id="aaCatchMapCount">${rows.length}</span>`:'🔒 Fangorte · PRO';countEl=q('#aaCatchMapCount');
    if(!pro){panel?.classList.add('hidden');clearMarkers();rows=[]}
  }
  async function updateAccess(){pro=await getPro();enabled=pro;syncAccessUi();if(pro)refresh()}
  function render(){
    const map=window.aaMap;if(!map)return;clearMarkers();if(!pro||!enabled)return;
    const species=select?.value||'',visible=rows.filter(r=>!species||String(r.species||'')===species);
    for(const c of visible){
      ...
      markers.push(L.marker([lat,lng],{icon:icon(),zIndexOffset:700}).addTo(map).bindPopup(popup,{maxWidth:260}))
    }
    if(countEl)countEl.textContent=String(visible.length);if(statusEl)statusEl.textContent=rows.length?`${visible.length} von ${rows.length} freigegebenen Fangorten im Kartenausschnitt.`:'In diesem Kartenausschnitt wurden noch keine Fangorte freigegeben.'
  }
  function syncSpecies(){if(!select)return;const current=select.value,names=[...new Set(rows.map(r=>String(r.species||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'de'));select.innerHTML='<option value="">Alle Fischarten</option>'+names.map(n=>`<option value="${safe(n)}">${safe(n)}</option>`).join('');if(names.includes(current))select.value=current}
  async function refresh(){
    const map=window.aaMap;if(!map||!enabled||!pro)return;clearTimeout(timer);timer=setTimeout(async()=>{
      if(!enabled||!pro)return;if(map.getZoom()<7){rows=[];syncSpecies();clearMarkers();if(countEl)countEl.textContent='0';if(statusEl)statusEl.textContent='Für Fangorte bitte näher heranzoomen.';return}
      const b=map.getBounds(),myToken=++token;if(statusEl)statusEl.textContent='Fangorte werden geladen …';
      try{
        const {data,error}=await sb.rpc('get_pro_catch_locations',{p_south:b.getSouth(),p_north:b.getNorth(),p_west:b.getWest(),p_east:b.getEast()});
        if(myToken!==token)return;if(error)throw error;rows=data||[];syncSpecies();render()
      }catch(e){if(myToken!==token)return;rows=[];clearMarkers();if(countEl)countEl.textContent='0';if(statusEl)statusEl.textContent='Fangorte sind nur mit aktivem AngelLog Pro verfügbar.'}
    },180)
  }
  function ensureUi(){
    if(button)return;const host=q('.aa-map-screen');if(!host)return;addStyles();const wrap=document.createElement('div');wrap.className='aa-catch-map-control';wrap.innerHTML=`<button id="aaCatchMapBtn" class="aa-catch-map-btn locked" type="button">🔒 Fangorte · PRO</button><div id="aaCatchMapPanel" class="aa-catch-map-panel hidden"><label for="aaCatchSpeciesFilter">Nach Fischart filtern</label><select id="aaCatchSpeciesFilter"><option value="">Alle Fischarten</option></select><label class="aa-catch-map-toggle"><input id="aaCatchMapEnabled" type="checkbox" checked> Fangorte auf Karte anzeigen</label><div id="aaCatchMapStatus" class="aa-catch-map-status">Pro-Fangorte werden geladen …</div></div>`;host.appendChild(wrap);
    ...
    button.onclick=async e=>{e.preventDefault();e.stopPropagation();if(!pro){await window.angelLogRequirePro?.('Fangorte und Fischartenfilter');return}panel.classList.toggle('hidden')};select.onchange=render;
    ...
    wrap.addEventListener('click',e=>e.stopPropagation());document.addEventListener('click',e=>{if(panel&&!panel.classList.contains('hidden')&&!wrap.contains(e.target))panel.classList.add('hidden')})
  }
  async function boot(){ensureUi();const map=window.aaMap;if(!map){setTimeout(boot,150);return}if(boundMap!==map){boundMap=map;map.on('moveend zoomend',refresh)}await updateAccess()}
  window.addEventListener('angelLog:entitlement',e=>{pro=!!e.detail?.isPro;enabled=pro;syncAccessUi();if(pro)refresh()});
  ...
  window.AngelLogCatchMap={refresh,render,updateAccess,get enabled(){return enabled},get pro(){return pro}}
})();