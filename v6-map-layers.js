(()=>{
  ...
  function updateChecks(){
    if(!menu)return;
    const base=window.getAngelLogMapStyle?.()||'osm';
    menu.querySelectorAll('[data-map-layer]').forEach(b=>{
      const v=b.dataset.mapLayer,on=v==='depth'?depthActive:(!depthActive&&v===base);
      const c=b.querySelector('.check');if(c)c.textContent=on?'✓':''
    })
  }

  function setStatus(msg,type=''){
    if(!statusEl)return;statusEl.textContent=msg;statusEl.className='aa-map-depth-status'+(type?' '+type:'')
  }

  function addAttribution(){
    const map=window.aaMap;if(!map||attributionAdded)return;
    try{map.attributionControl?.addAttribution('Tiefendaten © LfU Brandenburg');attributionAdded=true}catch{}
  }
  function removeAttribution(){
    const map=window.aaMap;if(!map||!attributionAdded)return;
    try{map.attributionControl?.removeAttribution('Tiefendaten © LfU Brandenburg')}catch{}attributionAdded=false
  }

  function clearDepth(){
    const map=window.aaMap;if(depthLayer&&map)try{map.removeLayer(depthLayer)}catch{}
    depthLayer=null;clearTimeout(refreshTimer)
  }

  function visibleDepthBounds(){
    const map=window.aaMap;if(!map)return null;const b=map.getBounds();
    const n=Math.min(b.getNorth(),BB.n),s=Math.max(b.getSouth(),BB.s),e=Math.min(b.getEast(),BB.e),w=Math.max(b.getWest(),BB.w);
    if(n<=s||e<=w)return null;return {n,s,e,w,b}
  }

  function refreshDepth(){
    if(!depthActive)return;const map=window.aaMap;if(!map)return;
    clearTimeout(refreshTimer);refreshTimer=setTimeout(()=>{
      if(!depthActive)return;
      const x=visibleDepthBounds();if(!x){clearDepth();setStatus(text('Hier sind noch keine amtlichen Tiefendaten angebunden.','No official depth data is connected here yet.'),'warn');return}
      ...
      const width=Math.max(320,Math.min(1100,Math.round(rect.width*((x.e-x.w)/fullW))));
      const height=Math.max(320,Math.min(1100,Math.round(rect.height*((x.n-x.s)/fullH))));
      const url=`/api/depth-map?n=${x.n.toFixed(6)}&s=${x.s.toFixed(6)}&e=${x.e.toFixed(6)}&w=${x.w.toFixed(6)}&width=${width}&height=${height}`;
      const bounds=L.latLngBounds([x.s,x.w],[x.n,x.e]);setStatus(text('Tiefenkarte wird geladen …','Loading depth map …'));
      ...
      next.once('error',()=>{try{map.removeLayer(next)}catch{};setStatus(text('Tiefendaten konnten für diesen Ausschnitt nicht geladen werden.','Depth data could not be loaded for this area.'),'warn')});
      next.addTo(map)
    },220)
  }

  function selectLayer(type){
    const map=window.aaMap;if(!map)return;
    if(type==='depth'){
