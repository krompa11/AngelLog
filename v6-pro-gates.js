(()=>{
  ...
  let pro=false,ready=false,applying=false;
  const PRO_STYLE_ID='aaProGateStyles';

  function styles(){
    if(q('#'+PRO_STYLE_ID))return;const s=document.createElement('style');s.id=PRO_STYLE_ID;s.textContent=`
  ...
  function tagButton(btn){
    if(!btn)return;btn.classList.toggle('aa-pro-ui-locked',!pro);let tag=btn.querySelector('.aa-pro-tag');
    if(!pro&&!tag){tag=document.createElement('span');tag.className='aa-pro-tag';tag.textContent='PRO';btn.querySelector('span:nth-child(2) b')?.appendChild(tag)}
    if(pro)tag?.remove()
  }
  function gateMap(){
    qa('[data-map-layer="satellite"],[data-map-layer="depth"]').forEach(tagButton);
    const layer=q('#aaLayerBtn');if(layer){layer.title=pro?'Satellit':'Satellit · PRO';layer.setAttribute('aria-label',layer.title);layer.classList.toggle('aa-pro-ui-locked',!pro)}
  ...
  function ensureGate(host,id,title,copy){
    if(!host)return;let el=q('#'+id);if(pro){el?.remove();return}
    if(!el){el=document.createElement('div');el.id=id;el.className='aa-pro-gate-card';el.innerHTML=`<b>🔒 ${title} · PRO</b><small>${copy}</small><button type="button">AngelLog Pro ansehen</button>`;el.querySelector('button').onclick=()=>requirePro(title);host.appendChild(el)}
  }
  function gateIntel(){
  ...
  function apply(){if(applying)return;applying=true;try{styles();gateMap();gateSettings();gateIntel()}finally{applying=false}}
  ...
  document.addEventListener('click',async e=>{
    ...
    if(advanced){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();await requirePro(advanced.matches?.('[data-map-layer="depth"]')?'Tiefenkarten':'Erweiterte Karten-Layer')}
  },true);
  document.addEventListener('change',async e=>{
    if(pro)return;
    if(e.target?.id==='setMap'&&e.target.value==='satellite'){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();e.target.value='osm';window.setAngelLogMapStyle?.('osm');await requirePro('Satellit und erweiterte Karten-Layer')}
    if((e.target?.id==='setExact'||e.target?.id==='setWaters')&&e.target.checked){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();e.target.checked=false;await requirePro(e.target.id==='setExact'?'Fangorte auf der Karte':'Pro-Benachrichtigungen für Lieblingsgewässer')}
  },true);

  window.addEventListener('angelLog:entitlement',e=>{pro=!!e.detail?.isPro;ready=true;apply()});
  const observer=new MutationObserver(()=>{if(ready)apply()});
  function boot(){styles();observer.observe(document.body,{childList:true,subtree:true});refreshAccess();setInterval(()=>{if(ready)apply()},1800)}
